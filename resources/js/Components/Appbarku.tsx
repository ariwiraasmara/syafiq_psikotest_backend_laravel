...
export default function Appbarku(props: Appbarku) {
    ...
    const linkBack = (isback: boolean, url: string) => {
        if(isback) {
            return (
                <Link rel="follow" title={`Kembali`} href={url} onClick={() => window.location.href= url} sx={{ marginRight : '10px', color: '#fff' }}>
                    <ArrowBackIcon />
                </Link>
            );
        }
